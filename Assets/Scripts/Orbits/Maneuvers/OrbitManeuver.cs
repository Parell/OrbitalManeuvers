        ...
        if (Application.isPlaying)
        {
            if (maneuvers.Count > 0)
            {
                ...
                else if (maneuvers[0].startTime <= 0)
                {
                    ...
                    {
                        OrbitController.Instance.orbitData[0].AddForce(maneuvers[0].direction.normalized, maneuvers[0].acceleration, Time.fixedDeltaTime * GameController.Instance.timeScale);

                        maneuvers[0].duration -= Time.fixedDeltaTime * GameController.Instance.timeScale;
                    }
